...
const initialCharacters = [
    { 
        ...
        traits: "Continental Army officer; patient, brave, front-line leader; carries a black rose in a leather pouch; haunted focus on the vampire killings; moves before dawn; admired/feared by the men."
    },
    { 
        name: "Pippin (the mouse)", 
        traits: "Close observer/POV; rides in Alaric\u2019s coat lining or saddlebag; wry, loyal, notices the man behind the iron mask."
    },
    { 
        name: "The Vampire (unnamed woman)", 
        traits: "Preternatural beauty; sings lullabies; leaves black-petaled rose on victims; intellect + hints of humanity; lethal and fast."
    }
];
...
const initialPlotBeats = [
    { 
        ...
        content: "Grisly crime scene; lullaby in the dark; first on-page reveal of the vampire; Kellers death underscores threat; Alaric recommits to the hunt."
    },
    { 
        title: "ACT I  Rising Action",
        content: "Reports of a family on the outskirts; black-rose signature; barracks/stables sequence; Alaric mounts the Friesian; Pippin frames Alarics dual nature."
    },
    { 
        title: "ACT I  First Clash / Near-Capture",
        content: "Alaric nearly corners her; her speed/intellect unsettle him; first crack of her humanity appears, complicating the hunt."
    }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const createNewCard = (title, content, type) => {
        ...
        // mirror external import field name for character-like items
    ...
    document.getElementById('add-character-card').addEventListener('click', () => {
        ...
        document.getElementById('add-location-card')?.addEventListener('click', () => {
            if (locationsContainer) locationsContainer.appendChild(createNewCard('Location Name', 'Key details…', 'location'));
        });
        document.getElementById('add-evidence-card')?.addEventListener('click', () => {
            if (evidenceContainer) evidenceContainer.appendChild(createNewCard('Clue', 'Where found, what it suggests…', 'evidence'));
        });
        document.getElementById('add-theme-card')?.addEventListener('click', () => {
            if (themesContainer) themesContainer.appendChild(createNewCard('Theme', 'How it appears in scenes…', 'theme'));
        });
    ...
    function initializeWarRoom() {
        ...
        function snapshot() {
            const serialize = (container) => [...container.querySelectorAll('.card')].map(card => ({
        ...
        function saveState() { localStorage.setItem('warroom', JSON.stringify(snapshot())); }
        function loadState() {
        ...
        // Attempt to load persisted state; if none, fall back to initializers
        if (!loadState()) { /* already populated by initializeWarRoom() */ }

        // Save whenever content changes or cards are added/removed
